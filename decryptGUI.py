...
class Application(Frame):
	...
	def create_widgets(self):
		...
		count_row = 0
		self.initialInstruction = Label(self, text = "Enter your encrypted message:", font=("Verdana", "12", "bold"))
		...
		count_row += 1
		self.cipherLabel = Label(self, text = "Choose the decrypt algorithm:", font=("Verdana", "12", "bold"))
		...
		self.alphabetLabel = Label(self, text="Which alphabet did you use?", font=("Verdana", "10", "bold"))
	...
	def run_algorithm(self):
		...
		if cipher == 1: # Affine Cipher
			...
			if valid is True:
				...
				self.result.insert(0.0, AffineCipher.decryptMessage(message, self.key, self.alphabet))
			else:
			...
			self.result.insert(0.0, CaesarCipher.decryptMessage(message, self.key, self.alphabet))

		elif cipher == 3: # Multiplicative Cipher
			valid = MultiplicativeCipher.validateKey(self.key, len(self.alphabet))
			if valid is True:
				# Continue the algorithm
				self.result.insert(0.0, MultiplicativeCipher.decryptMessage(message, self.key, self.alphabet))
			else:
				# Show a warning message
				self.warning["text"] = "This key can't be used with this alphabet.\nPlease enter another key."
				self.warning.grid(row=self.affine_row+6, column=11, sticky=W+W, padx=10, columnspan=2, rowspan=2)
			...
			if valid is True:
				# Continue the algorithm
				self.result.insert(0.0, MyCipher.decryptMessage(message, self.key, self.alphabet))
			else:
				# Show a warning message
				self.warning["text"] = "This key can't be used with this alphabet.\nPlease enter another key."
				self.warning.grid(row=self.affine_row+6, column=11, sticky=W+W, padx=10, columnspan=2, rowspan=2)
			...
			self.result.insert(0.0, TranspositionCipher.decryptMessage(message, self.key))
...
def main():
	...
	root = Tk()
	root.title("Reveal your messages!")
